...
def dal_create_course(cou_no, name, sex,bir,num,notes):
    with db_cursor() as cur : # 取得操作数据的游标，记为cur
        ...
        s = """
        INSERT INTO course (cou_sn, cou_no, name,sex,bir, notes) 
        VALUES (%(cou_sn)s, %(cou_no)s, %(name)s, %(sex)s,%(bir)s, %(notes)s)
        """
        cur.execute(s, dict(cou_sn=cou_sn, cou_no=cou_no, name=name, sex=sex, bir=bir, notes=notes))


def dal_update_course(cou_sn, cou_no, name, sex, bir, notes):
    with db_cursor() as cur : # 取得操作数据的游标，记为cur
        s = """
        UPDATE course SET
          cou_no=%(cou_no)s, 
